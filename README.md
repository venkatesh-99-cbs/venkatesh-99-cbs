<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:00FFAB,100:00D9FF&height=220&section=header&text=Venkatesh%20Tambabathula&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Cybersecurity%20Student%20%7C%20Python%20Developer%20%7C%20Security%20Research%20Enthusiast&descAlignY=55&descSize=18" width="100%"/>


<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&size=22&pause=1000&color=00FF9D&center=true&vCenter=true&width=900&lines=Security+Research+%7C+Python+Developer;Building+Secure%2C+Production-Grade+Applications;SOC+Analyst+in+Training;Engineering+for+Resilience%2C+Not+Just+Functionality" alt="Typing SVG"/>
</a>
<br>
<br>
<p>
  <img src="https://img.shields.io/badge/STATUS-ONLINE-00FF41?style=for-the-badge&labelColor=0D1117"/>
  <img src="https://img.shields.io/badge/LOCATION-Andhra%20Pradesh%2C%20IN-00D9FF?style=for-the-badge&labelColor=0D1117"/>
  <img src="https://img.shields.io/badge/OPEN%20TO-Internships%20%26%20Collaboration-FF005C?style=for-the-badge&labelColor=0D1117"/>
</p>

<p>
  <a href="#"><img src="https://img.shields.io/badge/RESUME-Download-00FF9D?style=for-the-badge&logo=readdotcv&logoColor=0D1117&labelColor=0D1117"/></a>
  <a href="#"><img src="https://img.shields.io/badge/PORTFOLIO-Visit%20Site-00D9FF?style=for-the-badge&logo=googlechrome&logoColor=0D1117&labelColor=0D1117"/></a>
  <a href="mailto:venkateshtambabathula99@gmail.com"><img src="https://img.shields.io/badge/EMAIL-Get%20in%20Touch-FF005C?style=for-the-badge&logo=gmail&logoColor=0D1117&labelColor=0D1117"/></a>
</p>

</div>

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## 🖥️ Digital Identity Card

```bash
┌──(venkatesh㉿cybersec-node)-[~/profile]
└─$ cat identity.json
```

```json
{
  "name"          : "Venkatesh Tambabathula",
  "alias"         : "venkatesh-99-cbs",
  "role"          : ["Cybersecurity Student", "Python Developer", "Security Research Enthusiast"],
  "education"     : "B.Tech — Computer Science Engineering (Cybersecurity)",
  "status"        : "🟢 Actively building & actively hacking (ethically)",
  "location"      : "Andhra Pradesh, India",
  "focus_areas"   : ["SOC Operations", "Threat Detection", "Incident Response", "AI for Security"],
  "current_mission": "Shipping Grow More — a production-grade Django e-commerce platform",
  "uptime"        : "99.9% curiosity-driven"
}
```

```bash
└─$ echo "connection secured. scroll to continue..." 
```

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## 👤 About Me

I'm a Cybersecurity-focused Computer Science student who learns best by building real systems and then trying to break them. My work sits at the intersection of **secure software engineering** and **offensive/defensive security** — I write the Django backend in the morning and run the Nmap scan against it in the evening.

- 🔭 Currently engineering **AI-assisted cybersecurity tooling** and **production full-stack web applications**
- 🛡️ Training in **SOC operations, threat detection, and incident response** as part of a structured cybersecurity program (Team: *Cyber Wizards*)
- 🧠 Believer in security-by-design — I treat authentication flows, payment webhooks, and file uploads as attack surfaces, not just features
- 🤝 Open to collaborating on **open-source security tooling, AI integrations, and full-stack builds**
- 💬 Ask me about **Python, Django, Linux internals, malware analysis basics, or full-stack architecture**

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## 🛣️ Cybersecurity Roadmap

```bash
└─$ tracker --module=cybersecurity --status=live
```

| Stage | Focus | Status |
|---|---|---|
| ✅ **Foundations** | Networking, Linux Administration, Python Scripting | `COMPLETE` |
| ✅ **Core Tooling** | Nmap, Wireshark, Metasploit, Kali Linux workflows | `COMPLETE` |
| 🟡 **In Progress** | SOC Operations, Threat Detection, Incident Response | `IN_PROGRESS ▓▓▓▓▓▓▓░░░ 70%` |
| 🟡 **In Progress** | Cloud Security (AWS/Render/Cloud-native deployments) | `IN_PROGRESS ▓▓▓▓▓░░░░░ 50%` |
| 🔵 **Up Next** | Applied AI for Security (LLM-assisted log/threat analysis) | `QUEUED` |
| 🔵 **Up Next** | Advanced Web App Penetration Testing | `QUEUED` |

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## ⚙️ Tech Stack

<div align="center">

**Languages**
<br>
<img src="https://img.shields.io/badge/Python-0D1117?style=for-the-badge&logo=python&logoColor=00D9FF"/>
<img src="https://img.shields.io/badge/Java-0D1117?style=for-the-badge&logo=openjdk&logoColor=00FFAB"/>
<img src="https://img.shields.io/badge/JavaScript-0D1117?style=for-the-badge&logo=javascript&logoColor=F7DF1E"/>
<img src="https://img.shields.io/badge/HTML5-0D1117?style=for-the-badge&logo=html5&logoColor=E34F26"/>
<img src="https://img.shields.io/badge/CSS3-0D1117?style=for-the-badge&logo=css3&logoColor=1572B6"/>

**Backend & APIs**
<br>
<img src="https://img.shields.io/badge/Django-0D1117?style=for-the-badge&logo=django&logoColor=00FF41"/>
<img src="https://img.shields.io/badge/Flask-0D1117?style=for-the-badge&logo=flask&logoColor=ffffff"/>
<img src="https://img.shields.io/badge/FastAPI-0D1117?style=for-the-badge&logo=fastapi&logoColor=00D9FF"/>
<img src="https://img.shields.io/badge/Node.js-0D1117?style=for-the-badge&logo=node.js&logoColor=6DA55F"/>
<img src="https://img.shields.io/badge/JWT-0D1117?style=for-the-badge&logo=jsonwebtokens&logoColor=ff5c5c"/>
<img src="https://img.shields.io/badge/Gunicorn-0D1117?style=for-the-badge&logo=gunicorn&logoColor=298729"/>

**Databases**
<br>
<img src="https://img.shields.io/badge/PostgreSQL-0D1117?style=for-the-badge&logo=postgresql&logoColor=00D9FF"/>
<img src="https://img.shields.io/badge/MySQL-0D1117?style=for-the-badge&logo=mysql&logoColor=4479A1"/>
<img src="https://img.shields.io/badge/MongoDB-0D1117?style=for-the-badge&logo=mongodb&logoColor=00FF41"/>

**Cybersecurity & Networking**
<br>
<img src="https://img.shields.io/badge/Kali_Linux-0D1117?style=for-the-badge&logo=kalilinux&logoColor=00D9FF"/>
<img src="https://img.shields.io/badge/Nmap-0D1117?style=for-the-badge&logo=nmap&logoColor=00FF41"/>
<img src="https://img.shields.io/badge/Wireshark-0D1117?style=for-the-badge&logo=wireshark&logoColor=1679A7"/>
<img src="https://img.shields.io/badge/Metasploit-0D1117?style=for-the-badge&logo=metasploit&logoColor=ff0000"/>
<img src="https://img.shields.io/badge/Linux-0D1117?style=for-the-badge&logo=linux&logoColor=FCC624"/>

**DevOps & Cloud**
<br>
<img src="https://img.shields.io/badge/Docker-0D1117?style=for-the-badge&logo=docker&logoColor=00D9FF"/>
<img src="https://img.shields.io/badge/Git-0D1117?style=for-the-badge&logo=git&logoColor=F05033"/>
<img src="https://img.shields.io/badge/GitHub_Actions-0D1117?style=for-the-badge&logo=githubactions&logoColor=2671E5"/>
<img src="https://img.shields.io/badge/Render-0D1117?style=for-the-badge&logo=render&logoColor=46E3B7"/>
<img src="https://img.shields.io/badge/Vercel-0D1117?style=for-the-badge&logo=vercel&logoColor=ffffff"/>
<img src="https://img.shields.io/badge/Netlify-0D1117?style=for-the-badge&logo=netlify&logoColor=00C7B7"/>
<img src="https://img.shields.io/badge/Firebase-0D1117?style=for-the-badge&logo=firebase&logoColor=FFCA28"/>

**Data / AI**
<br>
<img src="https://img.shields.io/badge/NumPy-0D1117?style=for-the-badge&logo=numpy&logoColor=00D9FF"/>
<img src="https://img.shields.io/badge/Pandas-0D1117?style=for-the-badge&logo=pandas&logoColor=ffffff"/>
<img src="https://img.shields.io/badge/scikit--learn-0D1117?style=for-the-badge&logo=scikitlearn&logoColor=F7931E"/>
<img src="https://img.shields.io/badge/PyTorch-0D1117?style=for-the-badge&logo=pytorch&logoColor=EE4C2C"/>
<img src="https://img.shields.io/badge/Matplotlib-0D1117?style=for-the-badge&logo=plotly&logoColor=ffffff"/>

</div>

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## 🎯 Skill Matrix

<div align="center">

<img src="https://quickchart.io/chart?bkg=%230D1117&width=420&height=420&devicePixelRatio=2&c=%7B%22type%22%3A%22radar%22%2C%22data%22%3A%7B%22labels%22%3A%5B%22Python%22%2C%22Django%22%2C%22Linux%22%2C%22Networking%22%2C%22Threat%20Detection%22%2C%22Web%20Security%22%2C%22Cloud%22%2C%22AI%2FML%22%5D%2C%22datasets%22%3A%5B%7B%22label%22%3A%22Skill%20Level%22%2C%22data%22%3A%5B90%2C85%2C80%2C75%2C65%2C70%2C55%2C60%5D%2C%22backgroundColor%22%3A%22rgba%280%2C255%2C157%2C0.25%29%22%2C%22borderColor%22%3A%22%2300FF9D%22%2C%22pointBackgroundColor%22%3A%22%2300D9FF%22%2C%22pointBorderColor%22%3A%22%2300D9FF%22%2C%22borderWidth%22%3A2%7D%5D%7D%2C%22options%22%3A%7B%22scales%22%3A%7B%22r%22%3A%7B%22angleLines%22%3A%7B%22color%22%3A%22%232a2f3a%22%7D%2C%22grid%22%3A%7B%22color%22%3A%22%232a2f3a%22%7D%2C%22pointLabels%22%3A%7B%22color%22%3A%22%2300FF9D%22%2C%22font%22%3A%7B%22size%22%3A12%7D%7D%2C%22ticks%22%3A%7B%22display%22%3Afalse%2C%22beginAtZero%22%3Atrue%7D%7D%7D%2C%22plugins%22%3A%7B%22legend%22%3A%7B%22display%22%3Afalse%7D%2C%22title%22%3A%7B%22display%22%3Afalse%7D%7D%7D%7D" width="380"/>


</div>

<br>

## 🧾 Professional Snapshot

<div align="center">

| 🎓 Education | 🛡️ Specialization | 🚀 Projects Shipped | 📜 Certifications | 🧑‍🤝‍🧑 Training Cohort |
|:---:|:---:|:---:|:---:|:---:|
| B.Tech CSE (Cybersecurity) | Security Engineering & Full-Stack Development | 8+ | 4 | NextGen Freedu — Cyber Wizards |

</div>

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## 🚀 Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### 🛒 Grow More — E-Commerce Platform
**Production-grade Django e-commerce system**

Multi-app architecture (`core`, `products`, `offers`, `orders`, `accounts`, `dashboard`) covering cart logic, wishlist, color/size variants, discount engine, and a custom admin product manager.

**Stack:** `Django` `PostgreSQL (Neon)` `Cloudinary` `WhiteNoise` `Render`

**Highlights:**
- 28+ resolved frontend/backend bugs across cart, wishlist & receipts
- Custom admin UI with color swatches and size/stock formsets
- Full production deployment pipeline (static + media + DB)

[`🔗 Repository`](https://github.com/venkatesh-99-cbs/grow-more) · [`🌐 Live Demo`](#)

</td>
<td width="50%" valign="top">

### 🔍 Security Log Analysis Assistant
**AI-assisted log triage tool**

A tool built to parse security/system logs and surface anomalies and likely indicators of compromise faster than manual review.

**Stack:** `Python` `Flask` `AI/LLM Integration`

**Highlights:**
- Structured log parsing pipeline
- AI-assisted anomaly flagging
- Built for SOC-style triage workflows

[`🔗 Repository`](https://github.com/venkatesh-99-cbs/security-log-analysis-assistant) · [`🌐 Live Demo`](#)

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🎓 Student Academic Management System
**Full-stack academic records platform**

A system for managing student records, academic tracking, and administrative workflows for an educational institution.

**Stack:** `Python` `Django/Flask` `SQL`

**Highlights:**
- Role-based access (admin/student views)
- Structured academic record management
- Clean, responsive UI

[`🔗 Repository`](https://github.com/venkatesh-99-cbs/student-academic-management) · [`🌐 Live Demo`](#)

</td>
<td width="50%" valign="top">

### 🌐 Personal Portfolio Website
**Dual-profile developer + cybersecurity site**

A multi-page portfolio (`index.html`, `developer.html`, `security.html`) presenting both full-stack development work and cybersecurity projects side by side.

**Stack:** `HTML5` `CSS3` `JavaScript` `Bootstrap`

**Highlights:**
- Separate developer & cybersecurity profile sections
- Custom UI for project and certification showcase

[`🔗 Repository`](https://github.com/venkatesh-99-cbs/portfolio) · [`🌐 Live Demo`](#)

</td>
</tr>
</table>

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## 🎓 Certifications

<div align="center">

<img src="https://img.shields.io/badge/Cisco-Introduction_to_Cybersecurity-00D9FF?style=for-the-badge&logo=cisco&logoColor=white&labelColor=0D1117"/>
<img src="https://img.shields.io/badge/Cisco-Linux_Unhatched-00FF41?style=for-the-badge&logo=linux&logoColor=white&labelColor=0D1117"/>
<br>
<img src="https://img.shields.io/badge/Cisco-Python_Essentials-FFD43B?style=for-the-badge&logo=python&logoColor=white&labelColor=0D1117"/>
<img src="https://img.shields.io/badge/TryHackMe-Networking_Rooms-FF005C?style=for-the-badge&logo=tryhackme&logoColor=white&labelColor=0D1117"/>

</div>

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## 📊 GitHub Analytics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=venkatesh-99-cbs&show_icons=true&theme=chartreuse-dark&hide_border=true&bg_color=0D1117&title_color=00FFAB&icon_color=00D9FF&text_color=c9d1d9" width="48%"/>
<img src="https://github-readme-streak-stats.herokuapp.com/?user=venkatesh-99-cbs&theme=dark&hide_border=true&background=0D1117&ring=00FFAB&fire=FF005C&currStreakLabel=00D9FF" width="48%"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=venkatesh-99-cbs&layout=compact&theme=chartreuse-dark&hide_border=true&bg_color=0D1117&title_color=00FFAB&text_color=c9d1d9" width="48%"/>
<img src="https://github-readme-activity-graph.vercel.app/graph?username=venkatesh-99-cbs&theme=react-dark&hide_border=true&bg_color=0D1117&color=00D9FF&line=00FFAB&point=FF005C" width="48%"/>

<img src="https://github-profile-trophy.vercel.app/?username=venkatesh-99-cbs&theme=algolia&no-frame=true&no-bg=true&margin-w=4&row=1"/>

<br>

<img src="https://github-readme-stats.vercel.app/api/pin/?username=venkatesh-99-cbs&repo=grow-more&theme=chartreuse-dark&hide_border=true&bg_color=0D1117&title_color=00FFAB&icon_color=00D9FF&text_color=c9d1d9" width="48%"/>
<img src="https://github-readme-stats.vercel.app/api/pin/?username=venkatesh-99-cbs&repo=portfolio&theme=chartreuse-dark&hide_border=true&bg_color=0D1117&title_color=00FFAB&icon_color=00D9FF&text_color=c9d1d9" width="48%"/>

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/output/github-contribution-grid-snake-dark.svg" width="100%"/>
<br>


</div>

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## 🧪 Currently Building

<table>
<tr>
<td align="center" width="25%">

🟢 **Threat Detection**
<br>
`ACTIVE`

</td>
<td align="center" width="25%">

🟡 **SOC Skills**
<br>
`IN_PROGRESS`

</td>
<td align="center" width="25%">

🟡 **Cloud Security**
<br>
`LEARNING`

</td>
<td align="center" width="25%">

🔵 **AI Security Research**
<br>
`EXPLORING`

</td>
</tr>
</table>

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

## 📡 Connect

<div align="center">

<a href="https://github.com/venkatesh-99-cbs">
  <img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=00FFAB"/>
</a>
<a href="https://linkedin.com/in/venkatesh-tambabathula">
  <img src="https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=00D9FF"/>
</a>
<a href="#">
  <img src="https://img.shields.io/badge/Portfolio-0D1117?style=for-the-badge&logo=firefox&logoColor=FF005C"/>
</a>
<a href="mailto:venkateshtambabathula99@gmail.com">
  <img src="https://img.shields.io/badge/Email-0D1117?style=for-the-badge&logo=gmail&logoColor=EA4335"/>
</a>

</div>

<br>

<div align="center">

<img src="https://komarev.com/ghpvc/?username=venkatesh-99-cbs&style=for-the-badge&color=00FFAB&label=PROFILE+VIEWS"/>

<br><br>

> *"The quieter you become, the more you are able to hear."* — every good SOC analyst, eventually

<br>

<img src="https://raw.githubusercontent.com/venkatesh-99-cbs/venkatesh-99-cbs/main/assets/scan-divider.svg" width="100%"/>

<sub>🟢 connection_terminated — session logged — `EOF`</sub>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00D9FF,100:0F2027&height=120&section=footer" width="100%"/>

</div>
